...
def _make_update_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        if "rev-parse" in joined and "--verify" in joined:
            return SimpleNamespace(
                stdout="",
                stderr="" if remote_branch_exists else "fatal: Needed a single revision\n",
                returncode=0 if remote_branch_exists else 128,
            )
...
# ---------------------------------------------------------------------------
# Origin-backed custom branches update in place; local-only branches fall back to main
# ---------------------------------------------------------------------------

def test_cmd_update_uses_origin_backed_feature_branch(monkeypatch, tmp_path, capsys):
    """When on an origin-backed feature branch, update pulls that branch."""
    _setup_update_mocks(monkeypatch, tmp_path)
    monkeypatch.setattr("shutil.which", lambda name: "/usr/bin/uv" if name == "uv" else None)
    ...
    assert pull_calls == [["git", "pull", "--ff-only", "origin", "fix/something"]]

    out = capsys.readouterr().out
    assert "switching to main" not in out


def test_cmd_update_switches_to_main_from_local_only_feature_branch(monkeypatch, tmp_path, capsys):
    """When on a local-only feature branch, update checks out main before pulling."""
    _setup_update_mocks(monkeypatch, tmp_path)
    monkeypatch.setattr("shutil.which", lambda name: "/usr/bin/uv" if name == "uv" else None)
...
def test_cmd_update_restores_stash_and_branch_when_already_up_to_date(monkeypatch, tmp_path, capsys):
    ...
    side_effect, recorded = _make_update_side_effect(
        current_branch="fix/something", commit_count="0", remote_branch_exists=False,
    )
    monkeypatch.setattr(hermes_main.subprocess, "run", side_effect)
